...
$(function() {
	var columns = [{
        ...
        },
        {
            ...
            formatter: function(value, row, index) {
            	var actions = [];
            	actions.push('<a class="btn btn-primary btn-sm" href="#" title="限时限价" mce_href="#" onclick="editconfig(\'' + row.id + '\',\'' + row.inventory + '\')"><i class="fa fa-plus"></i></a> ');
...
// 单条删除
function remove(id) {
	$.modalConfirm("确定要删除选中的预售款吗？", function(r) {
...
/*限时限量限价编辑*/
function editconfig(id,num) {
    var url = prefix + '/editconfig/' + id;
   layer_show("限时限量限价信息", url, '1000', '650');
}
